...
const Hero = ({ isDark }) => {
  ...
  const paragraph = "I'am Mohamed-Essam a Frontend developer focused on building high-impact websites, landing pages, and portfolio experiences using React. I combine clean UI, smooth animations, and real-world functionality to create interfaces that not only look sharp — but actually work, scale, and deliver results.";
        ...
        >
          {(() => {
            ...
            while (i < words.length) {
              ...
              if (word.toLowerCase() === 'drive' && nextWord?.toLowerCase() === 'real') {
                result.push(
                  <span key={i} className="text-gradient inline-block mr-4">
                    {`${word} ${nextWord}`.split('').map((char, j) => (
                      <span key={j} className="char inline-block whitespace-pre">{char}</span>
                    ))}
                  </span>
                );
                i += 2;
