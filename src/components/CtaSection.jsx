...
const floatingIcons = [
  { 
    ...
    color: "#25D366",
    label: "WhatsApp",
    position: "bottom-4 -left-4 sm:bottom-12 sm:-left-20 md:bottom-16 md:-left-24", // Cluster: Bottom Left
    delay: 0,
    duration: 3.2
  },
  { 
    ...
    color: "#E4405F",
    label: "Instagram",
    position: "-bottom-6 left-2 sm:-bottom-16 sm:-left-8 md:-bottom-20 md:-left-12", // Cluster: Bottom Left
    delay: 0.8,
    duration: 3.8
  },
  { 
    ...
    color: "#0077B5",
    label: "LinkedIn",
    position: "bottom-2 -right-4 sm:bottom-8 sm:-right-24 md:bottom-12 md:-right-28", // Cluster: Bottom Right
    delay: 1.5,
    duration: 4.2
  },
  { 
    ...
    color: "#6e5494",
    label: "GitHub",
    position: "-bottom-4 right-2 sm:-bottom-12 sm:right-4 md:-bottom-16 md:right-8", // Cluster: Bottom Right
    delay: 2.2,
    duration: 3.5
  },
  { 
    ...
    color: "#D4FF00",
    label: "CV",
    position: "-bottom-20 left-[10%] sm:-bottom-32 sm:left-[25%] md:-bottom-40 md:left-[30%]", // Cluster: Below Form
    delay: 3,
    duration: 4.5,
...
const CtaSection = ({ isDark }) => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      .fromTo(".cta-form-container", 
        { opacity: 0, scale: 0.95, y: 30 },
        { opacity: 1, scale: 1, y: 0, duration: 0.8, ease: "power2.out" },
        "-=0.4"
      )
      .fromTo(".mobile-icons-row", 
        { opacity: 0, y: 20 },
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[80vw] blur-[150px] opacity-20 rounded-full bg-dark-primary/15`} />
      </div>

      <div className="max-w-4xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          <h2 className={`cta-headline text-5xl md:text-8xl font-heading tracking-tighter uppercase leading-[0.9] mb-8 ${isDark ? 'text-white' : 'text-black'}`}>
            Let’s Build Something <br /> That Stands Out
          </h2>
          <p className={`cta-subtext font-body text-xl md:text-2xl max-w-2xl mx-auto leading-relaxed ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            I design and develop high-performing websites that engage users and drive real results.
          </p>
        </div>
              ...
              >
                <motion.a
                  ...
                  className={`flex items-center justify-center w-14 h-14 rounded-full border backdrop-blur-xl transition-all duration-300 ${
                    isDark ? 'bg-white/10 border-white/20 text-white/70 hover:text-white' : 'bg-black/5 border-black/10 text-black/50 hover:text-black'
                  }`}
                  style={{ 
          ...
          >
            <div className="grid grid-cols-1 gap-6">
              <div className="flex flex-col gap-2">
                <label className={`font-mono text-[10px] uppercase tracking-widest font-bold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                  Your Name
                </label>
                <input 
                  type="text" 
              ...
              >
                Send Message
                <Send size={20} className="transition-transform duration-500 group-hover:translate-x-1 group-hover:-translate-y-1" />
              </button>
            </div>
          </form>
        </div>
            ...
            <div key={idx} className="floating-icon-wrapper">
              <motion.a 
                ...
                className={`p-4 rounded-full border flex items-center justify-center transition-all duration-300 ${
                  isDark ? 'bg-white/10 border-white/10 text-white/70 hover:text-white' : 'bg-black/5 border-black/10 text-black/50 hover:text-black'
                }`}
              >
                {item.icon}
              </motion.a>
